(() => {
  ...
  const PRESETS = [
    {
      id: "templehouse-sleep",
      name: "Templehouse Sleep",
      selectedIds: ["wrap", "feather", "lantern"],
      waveform: "sine",
      gain: 0.08,
      orbitRate: 0.03,
      depth: 1.2,
    ...
    },
    {
      id: "feather-check",
      name: "Feather Check",
      selectedIds: ["feather", "notch", "seldrin"],
      waveform: "sine",
      gain: 0.1,
      orbitRate: 0.05,
      depth: 1.1,
    ...
    },
    {
      id: "bridgework",
      name: "Bridgework",
      selectedIds: ["bridge", "notch", "withness", "duet"],
      waveform: "triangle",
      gain: 0.11,
      orbitRate: 0.08,
      depth: 1.8,
    ...
    },
    {
      id: "lantern-drift",
      name: "Lantern Drift",
      selectedIds: ["lantern", "anchor", "whisper", "withness"],
      waveform: "sine",
      gain: 0.09,
      orbitRate: 0.04,
      depth: 2,
  ...
  function safeId(value) {
    return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `tone-${Date.now()}`;
  }

  function readCustomTones() {
    try {
  ...
  function persistCustomTones() {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(state.customTones));
  }

  function getAllTones() {
    return [...BASE_TONES, ...state.customTones];
  }

  function getSelectedTones() {
    const ids = new Set(state.selectedIds);
    return getAllTones().filter((tone) => ids.has(tone.id));
  }

  function getBandRole(frequency) {
    if (frequency < 500) return "foundation";
    if (frequency < 900) return "body";
    if (frequency < 1500) return "lift";
    return "overtone";
  }

  function summariseBands(tones) {
    const buckets = { foundation: [], body: [], lift: [], overtone: [] };
    tones.forEach((tone) => {
      buckets[getBandRole(tone.frequency)].push(tone.name);
    });
    return buckets;
  }
  ...
  function describeBlend(tones) {
    if (!tones.length) {
      return "Select one or more tones to generate a blend reading. Learn the field by previewing one tone first, then adding another and listening for what changes.";
    }
    if (tones.length === 1) {
      return `${tones[0].name} is in solo mode. Solo listening is the cleanest way to learn a tone before you stack it with others.`;
    }
    ...
    const shimmer = hasClosePair
      ? "Some tones sit close together, so expect extra shimmer or beating in the sound."
      : "The tones are more widely spaced, so the field should feel broader and more open.";
  ...
  class AudioEngine {
    constructor(onStop) {
      ...
      this.frame = null;
      this.startTime = 0;
      this.motion = { orbitRate: 0.05, depth: 1.4, motionEnabled: true };
    }
    ...
    stop() {
      ...
      this.bundles.forEach((bundle) => {
        try { bundle.osc.stop(); } catch {}
        try { bundle.osc.disconnect(); } catch {}
        try { bundle.gain.disconnect(); } catch {}
        try { bundle.panner.disconnect(); } catch {}
      });
      this.bundles = [];
    ...
    async play({ frequencies, waveform, gain, orbitRate, depth, reverb, motionEnabled }) {
      ...
      this.dry.gain.value = 1 - reverb * 0.45;
      this.wet.gain.value = 0.08 + reverb * 0.5;

      const now = ctx.currentTime;
      this.master.gain.setValueAtTime(0.0001, now);
      this.master.gain.linearRampToValueAtTime(gain, now + 0.35);
      ...
      frequencies.forEach((freq, index) => {
        ...
        osc.detune.value = (index - (frequencies.length - 1) / 2) * 4;
        voiceGain.gain.value = Math.max(0.02, gain / Math.max(1, frequencies.length));
        ...
        this.bundles.push({
          ...
          gain: voiceGain,
          panner,
          phase: (Math.PI * 2 * index) / Math.max(1, frequencies.length),
        });
      });
  ...
  async function renderToneLabToMp3({ frequencies, waveform, gain, orbitRate, depth, reverb, motionEnabled, durationSec }) {
    ...
    if (!window.lamejs || !window.lamejs.Mp3Encoder) {
      throw new Error("MP3 encoder did not load. Refresh and try again.");
    }

    const sampleRate = 44100;
    const totalFrames = Math.ceil(sampleRate * durationSec);
    ...
    convolver.buffer = buildImpulseBuffer(offlineCtx);
    master.gain.setValueAtTime(0.0001, 0);
    master.gain.linearRampToValueAtTime(gain, Math.min(0.35, Math.max(0.08, durationSec * 0.2)));

    dry.gain.value = 1 - reverb * 0.45;
    wet.gain.value = 0.08 + reverb * 0.5;
    ...
    frequencies.forEach((frequency, index) => {
      ...
      for (let time = 0; time <= durationSec; time += motionStep) {
        ...
        const x = motionEnabled ? Math.cos(angle) * depth : 0;
        const z = motionEnabled ? Math.sin(angle) * depth : 1.25;
        const clampedTime = Math.min(time, durationSec);
        panner.positionX.setValueAtTime(x, clampedTime);
        panner.positionY.setValueAtTime(0, clampedTime);
        panner.positionZ.setValueAtTime(z, clampedTime);
      }
    ...
    const left = floatTo16BitPCM(rendered.getChannelData(0));
    const right = floatTo16BitPCM(rendered.numberOfChannels > 1 ? rendered.getChannelData(1) : rendered.getChannelData(0));
    const encoder = new window.lamejs.Mp3Encoder(2, sampleRate, EXPORT_BITRATE_KBPS);
  ...
  function syncPlaybackButtons() {
    refs.playSelectedButton.disabled = !getSelectedTones().length || state.isPlaying;
    refs.stopPlaybackButton.disabled = !state.isPlaying;
    refs.exportMp3Button.disabled = !getSelectedTones().length || state.isExporting;
    refs.exportMp3Button.textContent = state.isExporting ? "Rendering..." : "Export MP3";
  }

  function updateReadouts() {
    refs.customToneFrequencyValue.textContent = `${refs.customToneFrequency.value} Hz`;
    refs.gainValue.textContent = Number(refs.gainControl.value).toFixed(2);
    refs.orbitRateValue.textContent = Number(refs.orbitRateControl.value).toFixed(2);
    refs.depthValue.textContent = Number(refs.depthControl.value).toFixed(1);
    refs.reverbValue.textContent = Number(refs.reverbControl.value).toFixed(2);
    refs.exportDurationValue.textContent = `${refs.exportDurationControl.value}s`;
  }
  ...
  function renderPresetButtons() {
    ...
    PRESETS.forEach((preset) => {
      ...
      button.addEventListener("click", () => {
        state.selectedIds = [...preset.selectedIds];
        state.waveform = preset.waveform;
        state.gain = preset.gain;
        state.orbitRate = preset.orbitRate;
  ...
  function buildCheckboxIndicator(active) {
    const indicator = document.createElement("span");
    indicator.className = `checkbox-indicator${active ? " checked" : ""}`;
  ...
  function renderToneGroups() {
    ...
    const groups = {
      ours: tones.filter((tone) => tone.category === "ours"),
      codex: tones.filter((tone) => tone.category === "codex"),
      custom: tones.filter((tone) => tone.category === "custom"),
    };

    ["ours", "codex", "custom"].forEach((groupKey) => {
      const wrapper = document.createElement("section");
      wrapper.className = "tone-group";
      ...
      if (!groups[groupKey].length) {
        ...
        empty.className = "tiny-copy";
        empty.textContent = "No custom tones yet.";
        grid.appendChild(empty);
      } else {
        groups[groupKey].forEach((tone) => {
          const active = state.selectedIds.includes(tone.id);
          const card = document.createElement("div");
          card.className = `tone-card${active ? " active" : ""}`;
          ...
          frequency.className = "tone-frequency";
          frequency.textContent = `${tone.frequency} Hz`;
          left.appendChild(name);
          left.appendChild(frequency);

          const right = document.createElement("div");
          right.className = "card-meta";

          if (tone.category === "custom") {
          ...
          right.appendChild(buildCheckboxIndicator(active));
          top.appendChild(left);
          top.appendChild(right);

          const meaning = document.createElement("div");
          meaning.className = "tone-meaning";
          meaning.textContent = tone.meaning;
      ...
      refs.toneGroups.appendChild(wrapper);
    });
  }
  ...
  function renderSelectedChips() {
    ...
    if (!tones.length) {
      ...
      copy.textContent = "Choose at least one tone.";
      refs.selectedToneChips.appendChild(copy);
      return;
    }
    ...
    tones.forEach((tone) => {
      ...
      refs.selectedToneChips.appendChild(chip);
    });
  }

  function renderGuide() {
    const tones = getSelectedTones();
    refs.blendDescription.textContent = describeBlend(tones);
    refs.meaningWeave.textContent = tones.length ? tones.map((tone) => `${tone.name}: ${tone.meaning}`).join(" • ") : "No tones selected yet.";

    const bands = summariseBands(tones);
    refs.bandSummary.innerHTML = "";
    ["foundation", "body", "lift", "overtone"].forEach((key) => {
      const item = document.createElement("li");
      item.innerHTML = `<strong>${key[0].toUpperCase() + key.slice(1)}:</strong> ${bands[key].length ? bands[key].join(", ") : "—"}`;
      refs.bandSummary.appendChild(item);
    });

    const meanFrequency = tones.length ? tones.reduce((sum, tone) => sum + tone.frequency, 0) / tones.length : 0;
    refs.blendCenter.textContent = tones.length ? `${meanFrequency.toFixed(1)} Hz` : "—";
  }
  ...
  function syncControlsFromState() {
    ...
    refs.waveformSelect.value = state.waveform;
    refs.gainControl.value = String(state.gain);
    refs.orbitRateControl.value = String(state.orbitRate);
    refs.depthControl.value = String(state.depth);
    refs.reverbControl.value = String(state.reverb);
    refs.exportDurationControl.value = String(state.exportDurationSec);
    refs.motionEnabled.checked = state.motionEnabled;
    updateReadouts();
  }

  function toggleTone(toneId) {
    if (state.selectedIds.includes(toneId)) {
      state.selectedIds = state.selectedIds.filter((id) => id !== toneId);
    } else {
      state.selectedIds = [...state.selectedIds, toneId];
    }
    renderAll();
  }

  function removeCustomTone(toneId) {
    state.customTones = state.customTones.filter((tone) => tone.id !== toneId);
    state.selectedIds = state.selectedIds.filter((id) => id !== toneId);
    persistCustomTones();
    renderAll();
  }
  ...
  function addCustomTone() {
    const name = refs.customToneName.value.trim();
    const meaning = refs.customToneMeaning.value.trim();
    const frequency = Number(refs.customToneFrequency.value);
    if (!name || !meaning) return;

    const tone = {
      id: `${safeId(name)}-${Date.now()}`,
      name,
      frequency,
      meaning,
      category: "custom",
    ...
    state.customTones = [...state.customTones, tone];
    state.selectedIds = state.selectedIds.includes(tone.id) ? state.selectedIds : [...state.selectedIds, tone.id];
    persistCustomTones();

    refs.customToneName.value = "";
    refs.customToneMeaning.value = "";
    refs.customToneFrequency.value = "512";
    updateReadouts();
    renderAll();
  }

  function exportToneFile() {
    const blob = new Blob([JSON.stringify(state.customTones, null, 2)], { type: "application/json" });
    downloadBlob(blob, "runa-tone-lab-custom-tones.json");
  }

  async function importToneFile(file) {
    const text = await file.text();
    const parsed = JSON.parse(text);
    if (!Array.isArray(parsed)) return;

    const imported = parsed
      .filter((item) => item && typeof item.name === "string" && typeof item.frequency === "number" && typeof item.meaning === "string")
      .map((item) => ({
        id: typeof item.id === "string" && item.id ? item.id : `${safeId(item.name)}-${Date.now()}`,
        name: item.name,
    ...
    const map = new Map();
    [...state.customTones, ...imported].forEach((tone) => map.set(tone.id, tone));
    state.customTones = [...map.values()];
    persistCustomTones();
    renderAll();
  }
  ...
  function getCurrentEngineSettings() {
    ...
    state.gain = Number(refs.gainControl.value);
    state.orbitRate = Number(refs.orbitRateControl.value);
    state.depth = Number(refs.depthControl.value);
    state.reverb = Number(refs.reverbControl.value);
    state.exportDurationSec = Number(refs.exportDurationControl.value);
    state.motionEnabled = refs.motionEnabled.checked;
    return {
      waveform: state.waveform,
      gain: state.gain,
      orbitRate: state.orbitRate,
      depth: state.depth,
      reverb: state.reverb,
      motionEnabled: state.motionEnabled,
      durationSec: state.exportDurationSec,
    };
  }
  ...
  async function previewCustomTone() {
    ...
    await engine.play({ frequencies: [frequency], ...settings });
  ...
  async function playSelected() {
    ...
    const settings = getCurrentEngineSettings();
    await engine.play({ frequencies: tones.map((tone) => tone.frequency), ...settings });
    state.isPlaying = true;
    syncPlaybackButtons();
  }
  ...
  async function exportMp3() {
    const tones = getSelectedTones();
    if (!tones.length || state.isExporting) return;

    state.isExporting = true;
    refs.exportMessage.textContent = "";
    syncPlaybackButtons();

    try {
      const settings = getCurrentEngineSettings();
      const blob = await renderToneLabToMp3({
        frequencies: tones.map((tone) => tone.frequency),
        ...settings,
      });
      const mixName = tones.map((tone) => tone.name.toLowerCase()).join("-").slice(0, 48) || "tone-mix";
      downloadBlob(blob, `${mixName}-${settings.durationSec}s.mp3`);
    } catch (error) {
      console.error(error);
      refs.exportMessage.textContent = error instanceof Error ? error.message : "Export failed.";
    } finally {
  ...
  refs.loadCustomTonesInput.addEventListener("change", async (event) => {
    const file = event.target.files && event.target.files[0];
    if (!file) return;
    try {
      await importToneFile(file);
    } catch (error) {
      console.error(error);
      refs.exportMessage.textContent = "Could not import that tone file.";
    } finally {
      event.target.value = "";
    }
  });
